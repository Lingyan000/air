...
const errorMsg = {
  broken: 'Air 配置文件损坏，已经恢复为默认配置',
  brokenButBackup: 'Air 配置文件损坏，已经恢复为备份配置',
};
...
function dbPathChecker(): string {
  ...
  try {
    ...
    const airLogPath = path.join(STORE_PATH, 'air.log');
    const logger = getLogger(airLogPath);
    if (!hasCheckPath) {
      const optionsTpl = {
        title: '注意',
...
function getGalleryDBPath(): {
  ...
  const dbPath = path.join(path.dirname(configPath), 'air.db');
  const dbBackupPath = path.join(path.dirname(dbPath), 'air.bak.db');
  return {
    dbPath,
    dbBackupPath,
  };
